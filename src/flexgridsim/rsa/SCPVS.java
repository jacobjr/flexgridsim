...
public class SCPVS implements RSA {
	...
	public void flowArrival(Flow flow) {
		...
		for (int i = 0; i < pt.getNumSlots()-demandInSlots; i++) {
			...
			id = vt.createLightpath(links, firstSlot, lastSlot);
			if (id >= 0) {
	...
	public ArrayList<FlexGridLink> getShortestPath(WeightedGraph G, int src, int dst, int demand) {
		...
		for (int i = 0; i < neighbors.length; i++) {
			...
			if (currentLink.contiguousSlotsAvailable(demand)){
				G.markAsVisited(v);
				tree.addLeaf(v);
				data.setAvailableSpectrum(1, v, currentLink.getFreeSlots());
		...
		while (L <= treeHeight){
			...
			for (int i = 0; i < leaves.size(); i++) {
				...
				for (int j = 0; j < neighbors.length; j++) {
					...
					if (pt.getLink(u, v).contiguousSlotsAvailable(demand) && cost < CR && !G.isVisited(v)){
						 tree.addLeaf(u, v);
						 G.markAsVisited(v);
						 boolean[] s1 = data.getAvailableSpectruim(L, u);
						 boolean[] s2 = pt.getLink(u,v).getFreeSlots();
